...
class LimsTemplate(models.Model):
    ...
    def create_lims_order_from_template(self, sale_order=None, sale_line=None):
        """Generate a lims.order record based on this template and optionally link it to Sale Order."""
        self.ensure_one()
        ...
        for test in self.test_ids:
            if hasattr(test, '_prepare_order_test_values'):
                ...
                values = {
                    'name': test.name,
                    'instrument_id': getattr(test, 'instrument_id', False) and test.instrument_id.id or False,
                    'company_id': test.company_id.id if test.company_id else self.company_id.id,
                }
